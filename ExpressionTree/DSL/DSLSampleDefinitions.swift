...
enum DSLSampleDefinitions {
	...
	// resolve against -- same content as the bundled
	...
	// Safe to coexist with the intrinsic lighting helpers a hand-written
	// node might also need in the same tree, because DSLCodegenNode
	// emits this module's functions under a name-mangled prefix (see
	// `mslModulePrefix`) instead of their plain names.
	...
	static let lightingModule: DSLResolvedModule = {
		do {
			...
			return DSLResolvedModule(qualifiedName: "lighting", module: module)
		} catch {
